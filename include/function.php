...
function get_zodiac_sign($month, $day)
{
    ...
    list($sign_start, $sign_name) = $signs[(int) $month - 1];
    if ($day < $sign_start)
        list($sign_start, $sign_name) = $signs[($month - 2 < 0) ? 11 : $month - 2];
    return $sign_name;
}
 ...
 * 
 * @param string $path ⽬录
 * @param string $suffix 匹配后缀
 * @return array
 */
function get_allfiles($path, $suffix = "")
{
    $files = [];
    if (is_file($path)) {
        if ($suffix !== "" && substr($path, 0 - strlen($suffix)) !== $suffix) {
            return [];
        }
        $files[] = $path;
    } elseif (is_dir($path)) {
        ...
        while ($file = $dp->read()) {
            if ($file != "." && $file != "..") {
                $files = array_merge($files, get_allfiles($path . "/" . $file, $suffix));
            }
        }
        $dp->close();
    }
