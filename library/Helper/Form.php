...
class Form
{
    ...
    public static function select($name, $options = array(), $selected = null, $attrs = array(), $add_empty = true)
    {
        $attrs['name'] = $name;
    ...
    public static function option($label, $value, $selected)
    {
        ...
        return '<option ' . Html::attributes($attrs) . '>' . Html::encode($label) . '</option>' . PHP_EOL;
    }
    ...
    public static function checkbox($name, $value, $attrs = array(), $checked_value = 1, $unchecked_value = 0)
    {
        ((string) $value == (string) $checked_value) ? $attrs['checked'] = 'checked' : null;
    ...
    public static function input_html($type = "submit", $name = null, $value = null, $attrs = array())
    {
        ...
        return '<input type="' . $type . '" name="' . $name . '" value="' . Html::encode($value) . '" ' . Html::attributes($attrs) . ' />' . PHP_EOL;
    }
    ...
    public static function button_to($name, $options = array(), $html_options = array())
    {
        ...
        if ($method)
            $js .= "var m = document.createElement('input');m.setAttribute('type', 'hidden');m.setAttribute('name', '_method');m.setAttribute('value', '$method');f.appendChild(m);";
        if ($token)
            $js .= "var s = document.createElement('input');s.setAttribute('type', 'hidden');s.setAttribute('name', 'authenticity_token');s.setAttribute('value', '$token');f.appendChild(s);";
        $js .="f.submit();";
        if ($confirm)
            $js .= "};";
        $js .= "return false;";
