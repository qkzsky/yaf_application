...
class Client
{
    ...
    public function __construct($server = '127.0.0.1:2379', $version = 'v3')
    {
        ...
        $this->baseUri = sprintf('%s/%s/', $this->server, $this->version);
    }
    ...
    protected function request($uri, array $params = [], array $options = [])
    {
        ...
        $httpClient = new \HttpClient($this->baseUri . $uri, 'post', 3000);
        $httpClient->setRequestHeader($data['headers'] ?? []);
        $httpClient->setRequestBody(json_encode($data['json']));
        $httpClient->exec();
